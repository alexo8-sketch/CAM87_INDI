...
begin
 ...
 while I >= 0 do
  begin
   ...
   if pos('CAM87',FT_Device_String) <> 0 then FT_flag:=true;    //if found cam87 - connect
  ...
  if FT_flag then
   begin
    if Open_USB_Device_By_Serial_Number(FT_HANDLEA,'CAM87A') <> FT_OK then FT_OP_flag := false;
    if Open_USB_Device_By_Serial_Number(FT_HANDLEB,'CAM87B')  <> FT_OK then FT_OP_flag := false;
